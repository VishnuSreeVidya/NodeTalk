...
export default function ChatWindow({ selectedUser, onStartCall }) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full app-container" style={{ background: 'var(--chat-bg)' }}>
      {/* Chat header */}
      <div className="p-4 border-b border-white/10 glass flex items-center gap-3 rounded-none">
        <div className="relative flex-shrink-0">
          <div className="w-10 h-10 rounded-xl glass-strong flex items-center justify-center text-sm font-bold">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="glass !p-2.5 !rounded-xl hover:bg-white/15 transition-all"
            title="Voice call"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {!loading && messages.length === 0 && (
          <div className="text-center text-white/30 py-12 text-sm animate-fade-in">
            ...
            >
              <div
                ...
                style={{
                  ...
                  border: isOwn ? '1px solid rgba(139,92,246,0.3)' : '1px solid rgba(255,255,255,0.06)',
                }}
              >
                {hasImage && (
                  <img
                ...
                <p className={`text-[10px] mt-1 ${isOwn ? 'text-white/40 text-right' : 'text-white/30'}`}>
                  {time}
                </p>
              </div>
            </div>
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="p-4 border-t border-white/10 glass rounded-none">
        <div className="flex items-center gap-2">
          <div className="relative">
            <button
              type="button"
              onClick={() => setShowEmoji(!showEmoji)}
              className="glass !p-3 !rounded-xl hover:bg-white/15 transition-all"
            >
              <svg className="w-5 h-5 text-white/70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
